...
function goodVsEvil(good, evil) {
   ...
   const goodPower = forceCalculated(good, goodArmy);
   const evilPower = forceCalculated(evil, evilArmy);
...
const forceCalculated = (force, army) => {
   const arrForce = force.split(' ');
   let powerOfForce = 0;

   for (let i = 0; i < arrForce.length; i++) {
      powerOfForce += arrForce[i] * army[i];
   }
